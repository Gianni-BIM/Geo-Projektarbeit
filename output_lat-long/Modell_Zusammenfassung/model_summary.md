...
Die bekannten Variablen sind fast identisch.
Dann sucht das Modell nach etwas anderem.

Wenn Punkt A und B in unterschiedlichen Regionen liegen, kann spatial_trend helfen.

Noch wichtiger:

`Spatial_trend` wird vor allem dann wichtig, wenn es systematische regionale Muster gibt.

Also etwa:

- Nordwesteuropa generell höhere SHI
...
- "An dieser Position erwarte ich SHI = 3.2"
- "An dieser Position erwarte ich SHI = 3.8"
- "An dieser Position erwarte ich SHI = 2.7"

Deshalb erhältst du kontinuierliche Zahlen.

## Man könnte spatial_trend vereinfacht so lesen:

"Egal was wir über Wetter, Landnutzung, Höhe usw. wissen: In dieser Gegend Europas sind die SHI-Werte tendenziell höher oder niedriger."

Aber mit einer kleinen Einschränkung:

Dein GAM wurde ja zunächst nur mit

`SHI ~ s(lon_x, lat_y)`

fitten.

Der daraus entstandene `spatial_trend` beschreibt also:

- "Wie hoch wäre der SHI allein aufgrund der geografischen Lage zu erwarten?"

Danach gibst du diesen Wert zusätzlich in den Random Forest.

Der Random Forest vergleicht dann:
...
und entscheidet selbst, wie viel zusätzliche Information `spatial_trend` noch liefert.

## Ein Beispiel

Stell dir vor, dein Modell sieht zwei Punkte:
...
Dann sagt das Modell:

>"Obwohl alle gemessenen Umweltvariablen gleich sind, liegt Punkt A in einer Gegend, in der SHI generell höher ist."
